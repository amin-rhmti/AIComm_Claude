...
\title[Intelligence or Interference?]{Intelligence or Interference? AI Agents and Online Social Interaction}
%\subtitle{Subtitle}
...
\begin{frame}{What determines $\mu$? (Cont'd)}
\begin{exampleblock}{Assumption 3}
    The parameter $\gamma$ depends on the number of human recipients.
    \begin{align*}
        \gamma \triangleq n_h.
    \end{align*}
\end{exampleblock}
...
\begin{itemize}
    ...
    \item In Case 2,
    \begin{align*}
        \mu_2 = \gamma\frac{m_h}{m_h+m_g}
    \end{align*}
\end{itemize}
\end{frame}

\begin{frame}{What determines $\lambda$?}
The social interaction parameter $\lambda$ takes a general form: $\lambda \triangleq \lambda(\vec{n},\vec{m})$. We assume the social interaction utility is \textcolor{orange0}{reciprocal}---``I engage more if you engage more." 
\begin{itemize}
    \item Since both human senders and Gen-AI senders chooses higher actions than rule-based senders, human or Gen-AI senders will make human recipients more engaged than rule-based senders. Thus an increase in $m_h$ or $m_g$ increases $\lambda$ more than an increase in $m_r$.
    \item Likewise, we assume an increase in $n_h$ or $n_g$ increases $\lambda$ more than an increase in $n_r$. 
    \item \textcolor{red}{(YL: we haven't model recipients' behavior here yet. So the above two are an assumption.)}
\end{itemize}

\vspace{1em}

\begin{exampleblock}{Assumption 4}
    The social interaction parameter $\lambda = \lambda(n_h,n_g,n_r,m_h,m_g,m_r)$ satisfies
    \begin{enumerate}
        \item $\frac{\partial \lambda}{\partial m_h}\geq \frac{\partial \lambda}{\partial m_r}$ and $\frac{\partial \lambda}{\partial m_g}\geq \frac{\partial \lambda}{\partial m_r}$
        \item $\frac{\partial \lambda}{\partial n_h}\geq \frac{\partial \lambda}{\partial n_r}$ and $\frac{\partial \lambda}{\partial n_g}\geq \frac{\partial \lambda}{\partial n_r}$
    \end{enumerate}
\end{exampleblock}
\end{frame}


\begin{frame}{What determines $\lambda$? (Cont'd)}
Interesting results we may expect from experimental outcomes:

\begin{enumerate}
    \item If $m_g$ is much smaller than $m_h$, then $\frac{\partial\lambda}{\partial m_g}>\frac{\partial\lambda}{\partial m_h}$. That is, a human recipient engages more if there are some Gen-AI senders.
    \item If $n_g$ is much smaller than $n_h$, then $\frac{\partial\lambda}{\partial n_g}>\frac{\partial\lambda}{\partial n_h}$. That is, a human sender prefers some Gen-AI recipients in the audience.
\end{enumerate}
