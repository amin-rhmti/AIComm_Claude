...
Let $v$ be drawn from a cdf $F\sim \Delta([\underline{v},\overline{v}])$
\vspace{1em}
...
Let $\Phi(v^*)\equiv v^* + \pi(v^*)T(v^*)$ be the human sender's total return from participating
\vspace{1em}

The threshold equilibrium with cutoff $v^*$ thus solves
\begin{align*}
    \Phi(v^*) = u_0+c
\end{align*}
...
\begin{exampleblock}{Lemma 1}
    In any stable equilibrium, $\Phi$ is increasing in $v^*$, i.e., $\Phi'(v^*)>0$. 
\end{exampleblock}
\end{frame}


\begin{frame}{Set of Equilibria}
\vspace{-.5em}
Let $a=0$ denote 'not participate' and $a=1$ denote 'participate'
\begin{exampleblock}{Proposition 1}
    When $\Phi$ is strictly increasing, then there is a unique equilibrium:
    \begin{itemize}
        \item if $u_0+c\in(\Phi(\underline{v}),\Phi(\overline{v}))$, $a=0$ for $v\in[\underline{v},v^*)$ and $a=1$ for $(v^*,\overline{v}]$, where $v^*$ solves $\Phi(v^*)=u_0+c$
        \item if $u_0+c>\Phi(\overline{v})$, $a=0$ for all $v\in[\underline{v},\overline{v}]$
        \item if $u_0+c<\Phi(\underline{v})$, $a=1$ for all $v\in[\underline{v},\overline{v}]$
    \end{itemize}
    When $\Phi$ is strictly decreasing, then 
    \begin{itemize}
        \item if $u_0+c\in (\Phi(\overline{v}),\Phi(\underline{v}))$, there are three equilibria: $a=0$ for all $v\in[\underline{v},\overline{v}]$, $a=1$ for all $v\in [\underline{v},\overline{v}]$, and an unstable equilibrium $a=0$ for $v\in [\underline{v},v^*)$ and $a=1$ for $v\in(v^*,\overline{v}]$ where $v^*$ solves $\Phi(v^*)=u_0+c$
        \item if $u_0+c>\Phi(\underline{v})$, there is a unique corner equilibrium with $a=0$
        \item if $u_0+c<\Phi(\overline{v})$, there is a unique corner equilibrium with $a=1$
    \end{itemize}
\end{exampleblock}
...
\begin{frame}{Comparative Statics of $v^*$}

\begin{exampleblock}{Proposition 2}
    Let $v^*\in (\underline{v},\overline{v})$ be an interior equilibrium cutoff with $\Phi'(v^*)>0$. Then
    \begin{enumerate}
        \item the comparative statics of $v^*$ wrt $\rho$ and $\alpha$ both depend on $T(v^*)$:
        \begin{itemize}
            \item if $T(v^*)<0$, then $\frac{\diff v^*}{\diff \rho}<0$ and $\frac{\diff v^*}{\diff \alpha}<0$
            \item if $T(v^*)>0$, then $\frac{\diff v^*}{\diff \rho}>0$ and $\frac{\diff v^*}{\diff \alpha}>0$
        \end{itemize}
        \item the cutoff $v^*$ increases in $\gamma$: $\frac{\diff v^*}{\diff \gamma}>0$
        \item the cutoff $v^*$ decreases in $\mu$: $\frac{\diff v^*}{\diff \mu}<0$
    \end{enumerate}
\end{exampleblock}
